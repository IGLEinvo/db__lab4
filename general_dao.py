...
class GeneralDAO(ABC):
    ...
    def find_all(self) -> List[object]:
        results = self._session.query(self._domain_type).all()

        # Виводимо результати до консолі
        print(results)
    ...
    def find_by_field(self, field_name: str, value: object) -> List[object]:
        query = self._session.query(self._domain_type).filter(getattr(self._domain_type, field_name) == value)
        results = query.all()
        return results
